...
mds_df2 <- mds_df %>%
  mutate(phase = case_when(Trt == "Control" ~ "Control",
         ...
         day_phase = case_when(Study_day == 0 ~ "Day 0 (Initial)",
                               Study_day == 12 ~ "Day 12 (Bloom)",
                               Study_day == 58 ~ "Day 58 (Climax)",
                               Study_day == 86 ~ "Day 86 (Decline)",
                               Study_day == 168 ~ "Day 168 (Recovery)",
                               Study_day == 376 ~ "Day 376 (Recovery)"),
         day_phase = factor(day_phase, levels = c("Day 0 (Initial)","Day 12 (Bloom)", "Day 58 (Climax)", "Day 86 (Decline)", "Day 168 (Recovery)", "Day 376 (Recovery)"), ordered = TRUE)) 
...
mds_day <- mds_df2 %>%
  ...
  geom_point(aes(color = day_phase, shape = trt), size = 5) +
  scale_shape_manual("Treatment", values = c(4, 16)) + 
